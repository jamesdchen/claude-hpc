...
compute the run_id, detect a resumable prior run, then assemble the validated
submit-flow spec — into a single workflow primitive that runs the branch logic
in code and returns one typed outcome plus a ready-to-submit context.
...
the submit.md Step 6a/6b-before-6c order.
...
full `BuildSubmitSpecInput` under `submit` (re-used verbatim so cluster /
profile / backend / job_env are not re-enumerated), and an optional
`BuildTasksPyInput` under `build_tasks` used only when `.hpc/tasks.py` is absent.
...
`cmd_sha`, the built `submit_spec` (on `resolved`), and the `prior_run_id` /
`prior_status` resume context (on `prior_run_found`). This is
escalation-as-data (#231): the composite runs every deterministic branch and
sets `needs_decision=True` only on `prior_run_found` (the user picks
resume-vs-fresh) and `needs_scaffold_interview` (the headless worker can't run
the scaffold sub-interview). `resolved` is the clean terminal — it hands back
the built spec, ready for [submit-pipeline](submit-pipeline.md) /
[submit-flow](submit-flow.md).

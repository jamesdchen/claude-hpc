...
run_abandoned, aggregate_failed, complete}` and a `needs_decision` flag, plus
...
| status `timeout` | `run_failed` | true | budget elapsed, jobs live — re-invoke to keep watching |
...
`timeout` maps to `stage_reached="run_failed"` deliberately — there is no
separate non-failed terminal in the literal, and an incomplete run cannot
aggregate. The `reason` field makes the budget-only nature explicit ("nothing
failed — budget only; re-invoke to keep watching") so the driver does not treat

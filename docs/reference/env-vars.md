...
| `HPC_SPEC_DUMP_DIR` | `<journal>/<repo_hash>/submit_specs/` | Designated folder for the pre-submit spec verification file (#212). `submit-flow --dry-run` writes the full resolved spec there as `<run_id>.json` (path returned as `spec_dump_path`) so a researcher can read + verify the parameters before any rsync/qsub. Default is kept out of the experiment tree so the dump never ships to the cluster; set this to a more visible / shared review folder if preferred (`hpc_agent.ops.submit.spec_dump`). |

...
| `HPC_AGENT_WORKER_JSON_SCHEMA` | (unset) | Set to `1`/`true` to spawn the delegated `claude -p` worker with `--json-schema` (the WorkerReport schema), constraining the worker's final report at **decode time** so malformed JSON can't be emitted — the structural complement to `parse_worker_report`'s cross-field checks (`hpc_agent._kernel.lifecycle.invoke`). Off by default until a live `claude -p --json-schema` run is validated against the `--bare` agent loop; when off, the worker uses the plain text transport. |

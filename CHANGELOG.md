...
### Added — spec verification / provenance dump (#212)

`submit-flow --dry-run` (and the auto-dispatched batch path) now writes the full resolved spec to a JSON verification file in a designated folder so a researcher can read + verify the parameters (grid, cluster, resources, walltime) before any rsync/qsub spends compute — the human-in-the-loop gate the issue asks for. A *real* submit also records the effective (defaults-resolved) spec to the same file, keyed by run_id, as a provenance artifact (best-effort in the flow's pre-rsync prelude, so a local write failure never blocks a cluster submission; campaign-driver submissions get it too, not just CLI). New `hpc_agent/ops/submit/spec_dump.py` (`spec_dump_dir` / `write_spec_dump`); the folder is `HPC_SPEC_DUMP_DIR` or, by default, `<journal>/<repo_hash>/submit_specs/` (kept out of the experiment tree so the dump never ships via rsync). On dry-run the absolute path is returned on the envelope as `spec_dump_path` (single) / `spec_dump_paths` (batch). The dry-run dump is the spec verbatim — a valid, re-submittable `submit-flow` spec. Complements the local *execution* gate (`dry-run-local` / #205): that proves the executor runs, this shows what it will run. `state/runs.py` grows a public `validate_run_id` (the single source for the filesystem-safe run_id guard `run_sidecar_path` already used) so the dump filename can't traverse out of its folder.

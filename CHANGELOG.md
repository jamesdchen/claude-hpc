...
### Performance — canary poll fast-start (submit & campaign pipelines)

- **The two-phase canary gate no longer dead-waits a full poll interval to observe a fast canary.** `verify-canary` polled at a flat `poll_interval_sec` (default 30s), so a 1-task canary that landed in ~5–10s was not *seen* for a full interval — pure latency on the critical path of every fresh submit (and every campaign iteration with a new `cmd_sha` the #249 canary cache can't skip). The poll loop now fast-starts at `HPC_CANARY_FAST_POLL_SEC` (default 3s) and ramps geometrically (3→6→12→24→30…) toward `poll_interval_sec`, which becomes the steady-state *ceiling* — a fast canary is caught in seconds while a slow one settles to the configured cadence, so steady-state SSH load is unchanged. Set `HPC_CANARY_FAST_POLL_SEC=0` to opt out. The "job vanished" verdict (#193) additionally requires the all-zero state to **span** a wall-clock grace (`= poll_interval_sec`) before firing, so the faster early polls can't falsely declare a slow-to-register canary `completed_unknown` and wrongly fail the gate. `ops/verify_canary.py`.

### Performance — `hpc-agent` CLI single-verb fast path

- **A known single verb no longer pays the full ~100-module registry import on startup.** `register_primitives()` eagerly imported every primitive-bearing module (~100 Pydantic-model modules, roughly half the process cold-start) on *every* invocation, but the common case dispatches one known verb that needs only its defining module. `main()` now consults a generated `cli/_verb_module_map.py` and, for an ungrouped handler-less verb, imports just that module (`register_single_module`) and builds a one-verb parser — a `monitor-flow` dispatch loads ~10 `_wire` + ~20 `ops` modules instead of ~50+/~40+. Self-healing and byte-identical: help/version, grouped/Tier-3/unknown verbs, an installed plugin, or any stale-map miss all fall back to the full path, so only speed differs. Kill switch `HPC_AGENT_NO_FAST_CLI=1`. The map is regenerated by `scripts/build_verb_module_map.py` (pre-commit + CI regen). `cli/dispatch.py`, `cli/parser.py`, `_kernel/registry/primitive.py`.

### Noted — remaining campaign-pipeline latency lever is async refill (#362, deferred)

- **The campaign loop's dominant remaining latency is its staged barrier**, not per-call overhead: iteration N+1 is not even proposed until iteration N fully drains (monitor + aggregate), so a pool of heterogeneous-duration trials idles on the slowest straggler at every batch boundary. The fix is already designed in [`docs/design/campaign-async-refill.md`](docs/design/campaign-async-refill.md) (#362): opt-in, K-in-flight continuous refill, recomputed from journal state each tick to preserve the crash-safety invariant. It is **deliberately not landed here** — its payoff is *not verifiable offline* (it needs a real cluster campaign with straggler-heavy trials), and the design doc records that two prior loop reshapes were ripped out for breaking the memory-stateless invariant. Tracked as the next, cluster-gated step.

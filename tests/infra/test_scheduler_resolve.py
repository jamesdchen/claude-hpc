...
# --- Phase 1b: qsub disambiguation (sge / pbspro / torque) -----------------


class DisambigSsh:
    """Fake ``ssh_run`` that models ``command -v`` for an arbitrary marker
    set plus a configurable ``qstat --version`` / ``qsub --version`` banner.

    Lets the disambiguation tree (sge vs pbspro vs torque) be exercised
    ...
    def __call__(self, cmd: str):
        ...
        if "-help" in cmd or "--version" in cmd:
            # qsub -help banner is generic context, not the disambig signal.

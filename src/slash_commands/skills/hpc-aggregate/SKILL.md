...
- **Be terse — caveman-compress your prose** (*strip grammar, keep facts*): drop articles, connectives, intensifiers, and filler; never restate tool output. **KEEP all facts verbatim** — run ids, counts, `hpc-agent <verb>` calls, `error_code`s, paths. One line beats three.

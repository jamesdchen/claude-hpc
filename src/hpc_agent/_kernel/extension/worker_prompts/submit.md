...
- `found=True` AND `status == "abandoned"` → a corpse the monitor gave up tracking, NOT a live prior (#276). Its `job_ids` are forensic, not in-flight. Proceed to 6d as if fresh — `submit-flow` re-submits over it (it no longer dedups against an abandoned record), and the canary refires rather than reusing the dead one. Do not surface a resume prompt.
- `found=True, is_orphan=False` (and `status` not `abandoned`) → real prior. Record a `prior_run` decision with outcome `found` (put the `run_id` in `why`) and surface to the caller — only the user can choose resume-vs-fresh.

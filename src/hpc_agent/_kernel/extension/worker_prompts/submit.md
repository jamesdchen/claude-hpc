...
Otherwise invoke [check-preflight](../../docs/primitives/check-preflight.md) with `--cluster <name>` — and, when the run is `runtime: uv`, ALSO pass `--runtime uv --modules "<MODULES>" --conda-source "<CONDA_SOURCE>" --conda-env "<CONDA_ENV>"` (the activation fields from Step 6d's `job_env`) so it runs the cluster-side `command -v uv` probe (#275 Fix 1) — the same check `submit-flow` runs internally, surfaced here at the cached gate so a uv-less env is caught before the canary, not at qsub. On `data.all_ok == true`: write/update marker, continue. On any check failure (including `runtime_uv_on_path`): do NOT write marker, record a `preflight` decision with outcome `setup_required`, put the failing checks verbatim in `anomalies`, and stop — the user fixes their environment with `hpc-agent setup --cluster <name>` and the caller re-invokes.

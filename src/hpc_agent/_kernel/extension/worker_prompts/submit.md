...
The Setup-step `action` already told you whether tasks.py exists: `reuse` / `interview` ⇒ present, `fresh` ⇒ absent. Confirm with a plain file test — do **not** import `hpc_agent` internals (`RepoLayout`, `load_tasks_module`) to look, and do **not** import `hpc_agent.state.run_sha` to hash it; 6c's `compute-run-id` owns the `cmd_sha`:

```bash
mkdir -p .hpc                                  # build-tasks-py (6b) also creates it; harmless if present
test -f .hpc/tasks.py && echo reuse || echo scaffold
```

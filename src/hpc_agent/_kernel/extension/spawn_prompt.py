...
def render_spawn_parts(
    *, workflow: str, experiment_dir: str, fields: dict[str, Any]
    ...
    cacheable_prefix = (
        ...
        "`hpc-agent describe`/`--help` lookups, separate greps — issue them as "
        "multiple tool-call blocks in ONE message (the harness runs them "
        "concurrently). 'Parallel' means parallel TOOL CALLS, not shell-level "
        "concurrency inside one Bash call (`cmd1 & cmd2 & wait`, `parallel`, "
        "`xargs -P`) — that trips the permission classifier as a compound "
        "command and interleaves output.\n"

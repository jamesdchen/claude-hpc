...
def _to_strict_schema(schema: dict[str, Any]) -> dict[str, Any]:
    ...
    **Invariant:** a strict-decoded output is always a *superset-constrained*
    ...
    ``$ref`` / ``$defs`` are preserved, not inlined: OpenAI strict and vLLM
    guided decoding both accept internal refs. ``format`` keywords known to
    be unsupported by strict mode are dropped (kept minimal).
    """
